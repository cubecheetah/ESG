...
# Corrplot
library(corrplot)
M <- cor(ESG[3:8]) # only usable if no columns were removed
...
ks.test(ESG$emp_nr, "pnorm", mean=mean(ESG$emp_nr), sd=sd(ESG$emp_nr)) # p value is smaller than 0.05 <- not normal distr

boxplot(ESG$ROA)
hist(ESG$ROA) # we should transform it ?
...
model_2 <- lm(ESG ~ ROA + mand, data = ESG)
summary(model_2)

model_3 <- lm(ESG ~ ROA + log(emp_nr) + Continent + Sector, data = ESG)
summary(model_3)
